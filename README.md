...
| **Validation** | `demo_slag_lp2_validation` | — | Does that L_P bias map hold on a **second** slag system at the same converter temperature? An independent set (Suito & Inoue 1984, Na₂O/BaO fluxes, 1550 °C, liquid low-C iron) says yes with an edge: on BaO slags Healy carries ~×1.5 — reproducing Drain from a different lab/decade — but on soda slags it under-predicts ~×4–5 at matched lime, because it reads only %CaO and is blind to Na₂O's basicity. The signed opposite of leg #1's high-lime over-prediction; a scientific bias-map extension (the engine carries no soda/baryta flux). |
| **Validation** | `demo_slag_ls_validation` | — | C_S was validated and L_P benchmarked — but does the sulfur **metal**-partition L_S the ladle desulfurization step actually computes carry? Against gas-controlled heats (Mohassab 2013, oxygen fixed by the gas — the only clean way to test the −log a_O term) it's **order-of-magnitude, not an upgrade**: the clean waterless CO/CO₂ subset under-predicts, and three confounds (a FeO-laden C_S baseline that is itself unvalidated, the −log a_O slope inseparable from FeO basicity, an a_O standard-state offset) mean the magnitude can't be pinned. Two edges survive — water raises L_S ~×5 (the engine is atmosphere-blind), and the FeO oxygen anchor reads ~×2 low. Both B3 residual gaps hit the same structural wall. |
...
| **P→DBTT coverage mechanism** (front-end) | `demo_p_segregation_dbtt` | — | *Why* is phosphorus' bulk contribution to the ductile-brittle transition (`grain.ITT_K_P`) a **flagged bracket** (~40–78 °C/0.1 wt% P), never a teeth-bearing coefficient? The clean measured form is grain-boundary **coverage → DBTT**, separated from the bulk-wt% term by two independently non-universal gaps: bulk P → coverage (a McLean isotherm that depends on thermal history — underdetermined, and it under-predicts absolute coverage, so a *sensitivity* leg only) and coverage → DBTT (cited but **per-steel**: 3.12 °C/at% in ferritic IF steel up to 13.31 in tempered-martensite SA508-4N — a 4.3× span, plus a 2× grain-size span). Composed, the bulk slope is a product of two non-universal factors whose ~20× range **contains** the 40–78 bracket with the flagged 50 mid-range — the mechanism explains the bracket's *width* but pins nothing. **No claimable tooth**: every coverage→DBTT relation is intergranular GB-segregation — a different fracture mode from the engine's transgranular-cleavage law — so none is an in-domain holdout; IF too (ferritic *matrix*) embrittles intergranularly, for lack of solute carbon, and is additionally the fitted steel. `grain.ITT_K_P` stays flagged and unchanged; the coverage relations are read, never applied (no engine touch, ADR 0010). |
