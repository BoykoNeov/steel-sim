...
| **Carbon carry-in** (front-end) | `demo_carbon_carry_in` | — | A *second* ladle mistake: the carbon the ferroalloys carry. Same charges, the ferroalloy carbon grade decides — cheap high-carbon (charge-grade) ferrochrome/ferromanganese (6–8 % C) drag a 4140 trim up ~+0.18 %C to ~0.56 %C, **off-grade on its own carbon band** and a harder steel (~700 HV vs the on-grade ~625); the refined low-carbon grades carry the same trim clean — *why low-carbon ferroalloys exist*. Off-grade fires on carbon through the same window machinery (no new flag); the hardness rise is the validated back end consuming the carry-in — propagation colour, not a verdict. No engine/ADR, **no claimable tooth** (mass-balance on cited assays); the ~40 %-of-the-grade's-carbon magnitude is an OoM coherence note. Deox-state recovery + P/S bands deferred. |
