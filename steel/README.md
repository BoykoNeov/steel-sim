...
| **F3** (Slice 1) | `ladle.py`, `demo_ladle.py`, `plots.ladle_figure` | **front-end ladle trim — alloy to grade by ferroalloy additions** (`docs/plans/steel-making.md` §7, build-order item 5). The seam where the composition the back end consumes is **finalized** — where the spine/F2 held the alloy fixed, F3 puts the Cr/Mo/Mn/Si *in*. **Spine-class** (no new physics): exact mixing/dilution mass-balance + a closed-form inverse (additions sized for an assumed recovery), with the cited **SAE J404 grade windows** (4140/8620) as a *labelled spec*. **The failure mechanism is the recovery shortfall:** additions sized for an assumed yield, delivered at a lower one (an under-killed bath eats the Cr/Mo — the F2 deox-state coupling) land **below the window** → F3 flags off-grade, and the *same* oil quench leaves a back-end **soft core** — one mistake, two flags (*at this section* off-grade fires first, the conservative early warning; the ordering is section-dependent — the deferred chemistry-spec ≠ H-band point). The under-dose is the spine's hero, now **produced** by a modeled ladle op, not hand-set. Teeth structural (round-trip, conservation); **named deferrals** = carbon carry-in from HC ferroalloys (~0.18 %C, quantified — built below) and the deox-state recovery coupling (built below) and P/S residual bands (no P/S state). No engine touch, no ADR | **built ✓** (2026-06-13) |
| **F3** (seam) | `ladle.py` (`oxidation_recovery_loss` / `recovery_after_deox`), `demo_deox_recovery.py`, `plots.deox_recovery_figure` | **front-end deox→recovery coupling — F2's dissolved oxygen taxes the oxidizable trim** (the **F2→F3 seam**; closes ladle's last named deferral). The dissolved oxygen F2 leaves on the `Heat` ties up a **stoichiometric mass** of the oxidizable additions as oxide (the same conservation `refining.generated_oxide` uses, read onto recovery), so their recovery falls below nominal — but **only Mn/Si** (the trim elements that carry a deox reaction in `refining.DEOXIDIZERS`); the noble Cr/Mo/Ni are oxygen-independent (**selectivity**, exact by construction). `trim_to_grade(couple_deox_recovery=True)` **produces** the recovery shortfall from the heat's `oxygen_ppm` instead of hand-setting it. **The honest magnitude is the result:** even a fully under-killed bath taxes Mn ~2 % at 4140's carbon (~4 % at 8620's lower carbon → higher dissolved O), so the landed Mn dips but stays *in-window* — the dissolved-O coupling **alone cannot trip off-grade**, which is quantitatively *why* `demo_ladle`'s gross under-trim hero must be hand-set. Carbon→oxygen→tax coherence ties F2's C–O coupling straight into F3 recovery. **No claimable tooth** (conservation arithmetic on cited oxide stoichiometry; the Si-over-Mn split a stated simplification); the gross slag-FeO **reoxidation distribution** (a metal→slag alloy partition `slag.py`'s `FeO` field could seed but does not) is the named ceiling. The only flag is F2's `porosity-risk`. No engine touch, no ADR | **built ✓** (2026-06-15) |
| **Capstone** (B2) | `demo_capstone.py`, `plots.capstone_figure` | **full-chain integration — one `Heat` threaded ore → billet → part** (`docs/plans/next-directions.md` B2). The synthesis the front and back halves were each built toward: the sealed seams (`from_hot_metal` → `decarburize` → `dephosphorize` → `deoxidize` → `degas` → `desulfurize` → `trim_to_grade` → `cast_billet` → `heat_treat`) composed on **one** un-rewritten provenance trail — until now the stages were only ever shown *independently* (`app_making`'s per-stage panels, each from its own fresh origin). Two heats, the identical chain, a **single knob** (the F2 blow endpoint): the reference (0.40 %C) lands a sound, on-grade, through-hardened part with the seeded tramp P/S driven below spec end to end; the over-blown foil (0.25 %C) is **off-grade** at the trim and **soft-cores** at the quench — the longest propagation in the repo, one mistake surfaced two stages apart. **Integration, no new physics** (the spine's posture — structural teeth: continuous trail, ordered field-fill, cross-chain propagation, and the reference clearing every sealed-engine spec). The lone front-end seam that takes a `Steel` (casting) is **re-based** onto the `Heat` via `evolve` (Option A, demo-local; promotion trigger = a second surface needing the same glue). Surfaced as `making.ipynb` §capstone (slice 2 — a static banked section reusing `demo_capstone`, so the re-base stays inside the demo: the promotion trigger is *not* tripped). No engine touch, no ADR | **built ✓** (2026-06-21) |
...
> `main()`. Per-stage panels only — chaining one `Heat` through the whole run is the **full-chain capstone**
> (`demo_capstone.py`, a separate headless demo — see the F-build table above), not this.
...
> seam → F4 casting + solidification), then the whole chain threaded onto one `Heat` end to end (§capstone),
> and then the nine defect consequences (through peritectic cracking
