...
#
# Under `-n auto` (pytest-xdist) each also carries @pytest.mark.xdist_group("heavy") — the
# shared group for the whole slow tail (test_demo_calphad.py + the notebook kernel test) — so
# every live solve lands on ONE worker (`--dist loadgroup`, set in pyproject addopts). That
# makes the module-scoped backends below build once per module and stops two heavyweight live
# solves running concurrently (oversubscription / the known multicomponent flake). The fast
# committed tests above are deliberately left ungrouped so they parallelise freely.
# =========================================================================== #
